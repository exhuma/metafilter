...
file_table = Table('file', metadata,
    Column('md5', String(32), ForeignKey('node.md5', onupdate="CASCADE", ondelete='CASCADE'), nullable=False, primary_key=True),
    Column('path', String, nullable=False, primary_key=True),
...
class Node(object):
    ...
    def is_dir(self):
        return self.mimetype == "inode/directory"

    def update_static_metadata(self):
        """
        Update the medatada that can be determined by the filesystem alone
        """
        if not self.files:
            return
    ...
    @classmethod
    def scan_path(self, sess, root, oldest_refresh=None, auto_tag_folder_tail = False, auto_tag_words=[]):
        import os
        ...
        for root, dirs, files in os.walk(root):
            ...
            for file in files:
                ...
                file = File(path)
                detached_file = Node()
                detached_file.files.append(file)
                detached_file.update_static_metadata()

                # TODO: remove the following line
                detached_file.metadata = 'a => x, c=>d';
                ...
                if oldest_refresh and detached_file.mod_time < oldest_refresh:
                    continue
...
mapper(File, file_table, properties={
    'node': relation(Node, backref='files')
    })
